...
```{r}
goal_3_and_4_scatterplot <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point() +
  theme_light () +
  scale_fill_viridis_d() +
...
ggplotly(goal_3_and_4_scatterplot)
```
...
leaflet(sdr_data_world_joined) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("Purples", goal_4_score)(goal_4_score), label = mytext)
```
